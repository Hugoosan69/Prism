...
- **Nenhum recurso pode depender só de `process.env`.** Em 23/09/2026 as variáveis de ambiente pararam de chegar ao processo em produção e cofre e busca na web se desligaram sem caminho de volta — enquanto Supabase (padrão versionado) e o modelo (chave no banco) seguiram de pé. Toda credencial de que um recurso depende precisa de ao menos uma rota que não passe pelo deploy. As do Drive vivem hoje em `settings` (`drive_app_id`, `drive_app_secret`, `drive_renewal`, `drive_folder`); `DriveVault` as recebe por parâmetro e quem decide se o cofre está ligado é `cofreLigado(settings)`.
- **Configuração do chat fica no banco** (`settings`, linha única): provedor, modelo e chaves são editáveis em `/configuracoes` e vencem sobre as variáveis de ambiente, que viram padrão de fábrica (`lib/ai/settings.ts`). Trocar de modelo não exige mais commit. As do Drive seguiram o mesmo caminho em 23/09
...
- Segredo nunca é versionado: o repositório é **público**. URL e chave publicável do Supabase têm padrão em `lib/supabase/config.ts` de propósito (quem protege é o RLS); `OPENAI_API_KEY`, `TAVILY_API_KEY` e as `GOOGLE_*` não são versionadas; todas elas podem ser substituídas pela tela de Configurações, e o `.env.local` é só o padrão de fábrica — ver `.env.example`
