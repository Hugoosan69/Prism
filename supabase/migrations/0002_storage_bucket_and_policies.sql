...
-- O repositório é público, então o endereço real não fica versionado: trocar
-- dono@exemplo.com pelo e-mail da conta antes de aplicar.

insert into storage.buckets (id, name, public) values ('files', 'files', false);

create policy "owner full access" on public.tasks for all to authenticated
  using ((auth.jwt()->>'email') = 'dono@exemplo.com') with check ((auth.jwt()->>'email') = 'dono@exemplo.com');
...
create policy "owner read files" on storage.objects for select to authenticated
  using (bucket_id = 'files' and (auth.jwt()->>'email') = 'dono@exemplo.com');
create policy "owner insert files" on storage.objects for insert to authenticated
  with check (bucket_id = 'files' and (auth.jwt()->>'email') = 'dono@exemplo.com');
